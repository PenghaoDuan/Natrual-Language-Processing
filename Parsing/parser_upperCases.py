# -*- coding: utf-8 -*-
"""
Created on Mon Oct  8 17:11:27 2018
    ...
    for node in t.bottomup():
        ...
        for children in node.children:
            ...
            if len(node.children) == 1:
                if children.label.lower() not in terminals:
                    terminals[children.label.lower()] = set()
                terminals[children.label.lower()].add(node.label)
        ...
        # Classifying wheteher it reaches leaves, if yes, storing the LOWERCASES
        # into rules
        if len(children_rule.split()) == 1:
            if children_rule.lower() in rules[node.label]:
                rules[node.label][children_rule.lower()] += 1
            else:
                rules[node.label][children_rule.lower()] = 1
